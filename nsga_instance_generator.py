...
def main():
    instance_list = generate(seed=0)
    generate_instance_files(path="instances", instance_list=instance_list)


def generate_instance_files(path="", instance_list=[]):
    for i, instance in enumerate(instance_list):
        os = open(f"{path}/{instance.factories}-{instance.machines}-{instance.num_jobs}__{i}.txt", mode='x')
        os.write(instance.to_text())
        os.close()
